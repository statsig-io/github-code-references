...
exports.replaceStaleConfigs = exports.replaceStaleGates = exports.searchConfigs = exports.searchGates = exports.parsePullRequestData = exports.getDynamicConfigsInFiles = exports.getFeatureGatesInFiles = exports.scanFiles = exports.getSpecificPartialGateRegex = exports.getSpecificFullGateRegex = exports.getGeneralGateRegex = exports.extensionToConfigRegexMap = exports.extensionToGatePartialRegexMap = exports.extensionToGateFullRegexMap = exports.SupportedFileExtensionsMap = exports.SupportedFileExtensionsSet = void 0;
const fs = require("fs");
...
const extensionIgnoreList = new Set([
    "git",
    "yaml",
    "yml",
    "json",
...
const SupportedExtensionsArray = ["ts", "py", "js", "tsx"];
exports.SupportedFileExtensionsSet = new Set(SupportedExtensionsArray);
exports.SupportedFileExtensionsMap = new Map(Array.from(SupportedExtensionsArray).map((ext) => [ext, ext]));
...
exports.extensionToGateFullRegexMap = new Map([
    ...
    [
        "py",
        /(?<lineStart>[\w _]*=)?[a-zA-Z _.]*check_gate\(.*, *['"]?(?<gateName>[\w _-]*)['"]?\) */i,
    ],
    [
        "tsx",
        /(?<lineStart>[{},\w_ ]*=)?[a-zA-Z_ .]*(?:use|check)Gate\([\w ,]*['"]?(?<gateName>[\w _-]*)['"]?\) *(?:.value)?;?/i,
...
exports.extensionToConfigRegexMap = new Map([
    ...
    [
        "tsx",
        /[a-zA-Z_ .]*[use]*Config\([\w ,]*['"]?(?<configName>[\w _-]*)['"]?\) *;?/i,
    ],
]);
...
async function scanFiles(dir) {
    ...
    while (queue.length > 0) {
        ...
        if (fs.lstatSync(currFileDir).isDirectory()) {
...
function searchGates(fileDir) {
    ...
    if (exports.SupportedFileExtensionsMap.has(extension)) {
        ...
        const regex = getGeneralGateRegex(exports.SupportedFileExtensionsMap.get(extension));
...
function replaceStaleGates(staleGates, fileDir) {
    ...
    const splitDir = fileDir.split(".");
    const extension = splitDir.at(-1);
    if (exports.SupportedFileExtensionsMap.has(extension) && staleGates.length > 0) {
        // Read within the file for the target string
        const fileData = fs.readFileSync(fileDir, "utf-8");
        let replacedFile = fileData;
        for (const staleGate of staleGates) {
            // Different languages, clients, servers have different ways of creating gates
            // Different regex target each instead of using one big regex blob
            const newString = extensionToGateReplace.get(extension);
            const fullRegex = getSpecificFullGateRegex(staleGate, exports.SupportedFileExtensionsMap.get(extension));
            const partialRegex = getSpecificPartialGateRegex(staleGate, extension);
            const gateMatch = replacedFile.match(fullRegex);
            if (gateMatch) {
                // If the stale gate has already been modified on the cleaned branch, then it should not match
                const matchedGroups = gateMatch.groups;
                if (!matchedGroups.lineStart) {
                    // if there is no start of the line, remove the entire line
