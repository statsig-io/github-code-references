...
const extensionIgnoreList = new Set<string>([
  "git",
  "yaml",
  "yml",
  "json",
...
const SupportedExtensionsArray = ["ts", "py", "js", "tsx"] as const;
export type SupportedFileExtensions = (typeof SupportedExtensionsArray)[number];
export const SupportedFileExtensionsSet: Set<SupportedFileExtensions> = new Set(
  SupportedExtensionsArray
);
export const SupportedFileExtensionsMap: Map<string, SupportedFileExtensions> =
  new Map(Array.from(SupportedExtensionsArray).map((ext) => [ext, ext]));
...
export function getGeneralGateRegex(extension: SupportedFileExtensions) {
...
export function getSpecificFullGateRegex(
  targetGate: string,
...
export function getSpecificPartialGateRegex(
  targetGate: string,
  extension: string
) {
  const gateCatchingGroup = "(?<gateName>[\\w _-]*)";
...
export function searchGates(fileDir: string) {
  ...
  if (SupportedFileExtensionsMap.has(extension)) {
    ...
    const regex = getGeneralGateRegex(
      SupportedFileExtensionsMap.get(extension)
    );
...
export function replaceStaleGates(staleGates: string[], fileDir: string) {
  ...
  if (SupportedFileExtensionsMap.has(extension) && staleGates.length > 0) {
    // Read within the file for the target string
    const fileData = fs.readFileSync(fileDir, "utf-8");
    ...
    for (const staleGate of staleGates) {
      ...
      const fullRegex = getSpecificFullGateRegex(
        staleGate,
        SupportedFileExtensionsMap.get(extension)
      );
...
export function replaceStaleConfigs(fileDir: string) {
  ...
  if (SupportedFileExtensionsMap.has(extension)) {
    ...
    const regex = new RegExp(
      extensionToConfigRegexMap.get(extension),
      REGEX_FLAG
    );
